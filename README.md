...
Builds tagged with special tags and master builds push files named `__sw-{prod,canary,int,qa}.js` to s3. Feature flags are then used to toggle which one is installed for which users. The relevant flags are
...
- All master builds release `/__sw-qa.js`
- All tags of the form `int-v{release number}` will release `/__sw-int.js`
...
To move a version of the service worker through one or more stages of the release cycle, care must be taken to tag the same commit with the related `int`, `canary` and `prod` tags
